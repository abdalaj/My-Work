 <script>
     ...
     $('input.qty,#unitPrice,.marketInput').on('keydown', function(e) {
         if (e.which == 13) {
             e.preventDefault();
             addRow();
         }
     ...
     $(document).on("change", "#personList", function(e) {
         ...
         if ($(this).val()) {
             $.ajax({
                 url: "https://app5.easyapps-eg.com/person/getDetails",
                 type: 'GET',
                 data: {
                     'p_id': $(this).val()
                 },
     ...
     $(document).on("change", ".salePersonSelect", function(e) {
         ...
         if ($(this).val()) {
             $.ajax({
                 url: "https://app5.easyapps-eg.com/getSalesDebt",
                 type: 'GET',
                 data: {
                     'sale_id': $(this).val()
                 },
     ...
     function addRow() {
             ...
             swal({
                 ...
                 function(){
                     ...
                     var data = '<td>'+
                         ...
                         num+
                         '</td>' +
                         '<td><input type="hidden" name="product['+num+'][store_id]" value="'+store_id+'">'+storeName+'</td>' +
                         '<td><input type="hidden" name="product['+num+'][product_id]" value="'+productID+'">'+productName+'</td>' +
                         '<td><input class = "itemQty tdinput" type="text" name="product['+num+'][qty]" value="'+productQty+'"></td>' +
                         '<td><input class="itemPrice tdinput" type="text" name="product['+num+'][price]" value="'+unitPrice+'"></td>' +
                         '<td>'+cloneUnitList+'</td>';
                         //'<td><input type="hidden" name="product['+num+'][unit_id]" value="'+unit_id+'">'+unitName+'</td>';
                                 ...
                                 '</td>';
                                                 data +='<td><input class="itemTotal tdinput" readonly type="text" name="product['+num+'][total]" value="'+total+'"></td>';
                                             data += '<td><input class="marketer" type="hidden" name="product['+num+'][markter]" value="'+marketer+'">'+marketer+'</td>' ;
                     ...
                     calculateTotal();
                     toastr.success("تمت إضافة "+productQty+" "+unitName+" للصنف "+productName+" من "+storeName+" للفاتورة");
         ...
         var data = '<td>' +
             ...
             productName + '</td>' +
             '<td><input class = "itemQty tdinput" type="text" name="product[' + num + '][qty]" value="' + productQty +
             '"></td>' +
             '<td><input class="itemPrice tdinput" type="text" name="product[' + num + '][price]" value="' + unitPrice +
             '"></td>' +
             '<td>' + cloneUnitList + '</td>';
         @if ($settings['use_bounse'] == 1)
             data += '<td>' +
                 ...
                 '</td>';
         @endif
         data += '<td><input class="itemTotal tdinput" readonly type="text" name="product[' + num + '][total]" value="' +
             total + '"></td>';
