                ...
                if($event->order->status=='delivered'){
                    if($event->order->invoice_type=='sales'){
                        ...
                        if ($orderQty > $avilableQty && $checkqty==2) {
                            //\Log::info('avilableQty ='.$item->id.' '.$avilableQty);
                            throw new \Exception('الكمية غير متاحة '.$item->pivot->product_name.' فى '.$item->pivot->store_name);
                        }
    ...
    public function unitPriceCalaculation($item,$storUnit){
        ...
        foreach ($productUnit as $punit){
            ...
            if($categoryPercentage && $item->is_price_percent){
                ...
                $price1 = $saleprice;
                if($storUnit->gomla_price && $item->category->percentage2)
                    $price2 = $productCost + ($productCost * ($item->category->percentage2/100));
